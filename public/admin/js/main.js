...
let addHotelBtn = document.querySelector('.create-hotel-btn');
...
addHotelBtn.addEventListener('click', function(){
    let hotelsTab = document.getElementById('v-pills-hotel');
    hotelsTab.classList.remove('show');
    hotelsTab.classList.remove('active');

    let createhTab = document.getElementById('v-pills-create-hotel');
    createhTab.classList.add('show');
    createhTab.classList.add('active');
...
async function addHotels(){
    ...
    //posts: we have an array of all hotel stored in the DB.
    let hoteldetails = document.querySelector('.hoteldetails');
    /*we have to be sure that every time we work with the hotels,
    this div is empty without any articles*/
    ...
    <article class="d-flex justify-content-between align-items-center article-inline">
        <div class="num w5">${i++}</div>
        <input class="id" type="hidden" value="${hotel.id}">
        <div class="name w30">${hotel.title}</div>
        <div class="date w30">${hotel.date}</div>
        <div class="country w20">${hotel.country}</div>
        <div class="edit w10"><button class="btn btn-link btn-edit">Edit</button></div>
        <div class="remove w5"><button class="btn btn-link btn-remove">X</button></div>
    </article>`;
    //Let's add some articles
    hoteldetails.insertAdjacentHTML('beforeend', hotelHTML);
    })
}
